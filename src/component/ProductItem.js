    ...
    return props.products.map(function(product){
        return(
            <div className="product-item" id={product.id}>
                <div className="product-image"><img style={{height:"300px"}} src={product.image} alt={product.name}/></div>
                <div className="product-name" onMouseEnter={() => {setShowDescription(true); setProductHover(product.id)}} onMouseLeave={() => setShowDescription(false)}>{product.name}</div>
